...
NUM_EPOCHS = 1200 // NUM_LABELLED_VOLS
CHECKPOINT_FREQ = max(1, NUM_EPOCHS // 5)

LAMBDAS = [
    0.02 * np.concat((
        lin_ramp(0.15, 0.0, 0.5, 1.0, np.linspace(0.0, 0.5, max(NUM_EPOCHS // 4, 1))),
        lin_ramp(0.67, 1.0, 1.0, 1.0, np.linspace(0.6, 1.0, max(NUM_EPOCHS - NUM_EPOCHS // 4, 1)))
    )),
    np.zeros(int(NUM_EPOCHS)),
]
...
def get_dataset(paths, img_size=IMG_SIZE, shuffle_buf=None):
    """Load dataset. Much of this function was generated by ChatGPT.
    If it works I claim credit but if it doesn't then its all ChatGPT's fault.

    Args:
        paths (list): List of paths to slices to load.
        img_size ((int, int), optional): Image resolution to downsample to. Defaults to IMG_SIZE.
        shuffle_buf (int, optional): Size of shuffle buffer. Defaults to None.

    Returns:
        any: Tensorflow dataset
    """    
    ...
    def _read_h5(path):
        ...
        with h5.File(path, "r") as f:
            ...
            # ^ Only using 3 modalities so we can pretend RGB and use MobileNetV2 pretrained
...
# Courtesy of ChatGPT - I also checked for correctness
...
def unsupervised_loss_func(xu_batch, model, num_channels=3):
    if xu_batch == None:    # Happens if DONT_LOAD_UNLABELLED is set.
        return 0.0
